...
filename = '003.k_fold_validation_split_2'
...
feat_names = pickle.load(open('data/feat_names_3.sav', 'rb'))

feat_names_for_val = list((set(feat_names) - set(['mean_enc_supply', 'mean_enc_region', 'mean_enc_cond'])))

data_mart = train_df[feat_names_for_val]
...
for tr_idx, ts_idx in kfold.split(data_mart):
    ...
    for sub_tr_idx, sub_ts_idx in kfold.split(tr_df):
        ...
        sub_ts_df['mean_enc_region'] = sub_ts_df['지역'].map(sub_tr_df.groupby('지역')['등록차량수'].mean())
        sub_ts_df['mean_enc_cond'] = sub_ts_df['자격유형_merge'].map(sub_tr_df.groupby('자격유형_merge')['등록차량수'].mean())
    ...
    tr_df['mean_enc_region'].fillna(global_mean, inplace = True)
    tr_df['mean_enc_cond'].fillna(global_mean, inplace = True)
...
pickle.dump(train_val_list,
            open('data/train_val_list_3.sav', 'wb'))
